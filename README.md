# Juan Manuel Pérez Rodríguez (Jmprdeveloper)
Técnico Superior en ASIR | Sistemas, redes y soporte | Desarrollo de software (Android / escritorio)

## App publicada en Google Play
**Decompras (Android)** — Publicada como **Jmprdeveloper**  
https://play.google.com/store/apps/details?id=com.juanma.planificador&hl=es

## Perfil profesional
Experiencia en gestión integral de entornos TI en consultoría: administración de **Windows Server** y **Linux**, redes (**TCP/IP, DNS, DHCP, VPN, firewall**), soporte a usuarios, mantenimiento, copias de seguridad y coordinación con proveedores.  
Complemento el perfil de sistemas con desarrollo de aplicaciones, aplicando un enfoque práctico, orientado a resultados y mejora continua.

## Proyectos
### Decompras (Android) — Google Play
- Enlace: https://play.google.com/store/apps/details?id=com.juanma.planificador&hl=es  
- Publicación y mantenimiento en Google Play (ciclo completo).

### OmniPlanner (Android)
- Planificador/organización personal (en desarrollo).

### SinHumo (Android)
- Seguimiento de hábitos para dejar de fumar (en desarrollo).

## Tecnologías
- Sistemas: Windows Server, Linux, redes (TCP/IP, DNS, DHCP, VPN, firewall)
- Desarrollo: Kotlin/Java (Android), bases en C# y Python, WordPress/HTML/CSS

## Contacto
- GitHub: https://github.com/Jmprdeveloper
